@php
//dd(formIterator(($personExperience ?? null), 'experience'));
@endphp

@extends('layouts.layoutTutorials')

@section('headTitle')
    Laravel Collective Forms Example
@endsection

@push('scripts')
    <script src="{{ mix('/build/compare-forms/laravel.add.js') }}"></script>
...
@section('content')
   ...
   <p>
    ...
    {!! Form::model($personExperience, [
                    'route'=>['compare.laravel.update', 'id'=>$personExperience->id, app()->getLocale()], 
                    'method'=>'put', 
                    'id'=>"experience", 
                    'class'=>"editForm" 
        ]) !!}
    @else
    {!! Form::open(['route'=>['compare.laravel.store', 'locale'=>app()->getLocale()], 'method'=>'post', 'id'=>"experience", 'class'=>"editForm" ]) !!}
    @endif

    @if ($errors->any())
        <p class="errors">@lang('dynamicForms.formErrors')</p>
    @endif
    ...
    @enderror
        <div
            ...
            data-template="@attributeTemplate('compare-forms.laravel._experience', ['k'=>'__index1__', 'countries'=>$countries, 'languages'=>$languages, 'workTypes'=>$workTypes])"
        ...
        >
            ...
            <div class="row">
                ...
                <div class="input-col">
                    ...
                    <table
                        ...
                        data-template="{!! attributeTemplate('compare-forms.laravel._list-row', ['name'=>'additionalLanguages.__index__', 'nameAttribute'=>'additionalLanguages[]', 'placeholder'=>trans('messages.Programming Language')]) !!}"
                    >
                        <tbody>
                        @foreach(formIterator(($personExperience ?? null), 'additionalLanguages') as $key)
                            @include('compare-forms.laravel._list-row', ['name'=>'additionalLanguages['.$key.']', 'nameAttribute'=>'additionalLanguages[]', 'placeholder'=>trans('messages.Programming Language')])                        
                        @endforeach
                        </tbody>
                        <tfoot>
                            <tr>
            ...
            @foreach (formIterator(($personExperience ?? null), 'experience') as $key)
                @include('compare-forms.laravel._experience', ['k'=>$key, 'countries'=>$countries, 'languages'=>$languages, 'workTypes'=>$workTypes])
            @endforeach
        </div>
